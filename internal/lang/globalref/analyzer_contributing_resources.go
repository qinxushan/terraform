...
// for each one tries to walk backwards through any named values to find
...
func (a *Analyzer) ContributingResources(moduleAddr addrs.ModuleInstance, refs ...Reference) []addrs.AbsResource {
	...
	found := make(map[string]addrs.AbsResource)
	...
	modCfg := a.cfg.DescendentForInstance(moduleAddr)
	if modCfg == nil {
		// Weird! We can't work with this.
		return nil
	}
	...
	ret := make([]addrs.AbsResource, 0, len(found))
